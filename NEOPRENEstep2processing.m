...
for k=1:length(conditions) % loop with preprocessed .set reload, because eeglab segments 1 event type at a time
    ...
    for diri=1:num_files
        ...
        % there are no control trials for sub-17CE and sub-20DD at 0 YO, and for sub-61AT at 2 YO
        if (((~contains(subject,'2Y')) && ((contains(subject,'sub-17CE'))||(contains(subject,'sub-20DD'))))...
                ||((contains(subject,'2Y')) && (contains(subject,'sub-61AT'))))...
                && (contains(conditions{k},'Control'))
        continue; end 
...
% concatenate segments from the different ISI groups, for each prefix group
