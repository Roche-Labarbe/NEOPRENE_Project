...
for diri=1:num_files
    ...
    %% filtering
    EEG = pop_eegfiltnew(EEG, 'locutoff',0.5,'hicutoff',30); % bandpass
    ...
    % log the number of components rejected
    logICA(diri).subject=subject;
    logICA(diri).nRejected = sum(EEG.reject.gcompreject);
...
save('ICA_log.mat', 'logICA'); % save the number of ICA components rejected for each subject

eeglab redraw; % updated the EEGlab GUI and its variables