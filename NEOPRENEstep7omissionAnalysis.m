...
for subi=1:size(Omvalues,2)
    for factori=1:size(Factors,1)
        if contains(Factors.Subject(factori),Omvalues.Properties.VariableNames{subi})
            Omvalues{'GA',subi}=Factors.GA(factori);
            Omvalues{'Pain',subi}=Factors.Pain(factori);
            Omvalues{'log(Pain)',subi}=Factors.logPain(factori);
            Omvalues{'StimSide',subi}=Factors.StimSide(factori);
            Omvalues{'Sex',subi}=Factors.Sex(factori);
            Omvalues{'SDSC',subi}=Factors.SDSC(factori);
            ...
            Omvalues{'PC1_Actim',subi}=Factors.PC1_Actim(factori);
        end
    end
end
