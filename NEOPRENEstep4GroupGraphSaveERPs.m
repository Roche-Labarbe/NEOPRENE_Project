...
GrandAvgContraFrontalERPOtherStd=table('RowNames',string(time)); GrandAvgContraS1ERPOtherStd=table('RowNames',string(time));
GrandAvgIpsiFrontalERPOtherStd=table('RowNames',string(time)); GrandAvgIpsiS1ERPOtherStd=table('RowNames',string(time));
...
for diri=1:num_files
    ...
    if ~contains(Famfilename,'sub-20DD') && ~contains(Famfilename,'sub-17CE')...
            && ~contains(Famfilename, '2Y_') % sub 17 and 20 at 0Y do not have Control markers because of bug during acquisition
        ...
        GrandAvgContraFrontalERPControl.(subject)(:)=NaN; GrandAvgContraS1ERPControl.(subject)(:)=NaN;
        GrandAvgIpsiFrontalERPControl.(subject)(:)=NaN; GrandAvgIpsiS1ERPControl.(subject)(:)=NaN;
...
if ~contains(Famfilename, '2Y_')
    graphylimit=1;
elseif contains(Famfilename, '2Y_')
    graphylimit=2;
end
...
xlabel('Time (ms)'); ylabel('Amplitude (ƵV)'); yline(0); ylim([-graphylimit graphylimit]);  xlim([-300 800]); xline(0); xline(200); axis ij;
...
xlabel('Time (ms)'); ylabel('Amplitude (ƵV)'); yline(0); ylim([-graphylimit graphylimit]);  xlim([-300 800]); xline(-200,':'); xline(0,':'); xline(200,':'); axis ij;
...
%% Other standards : grand average graph of other standards

file_list=dir('*_Other standard_avgERPs.mat');
num_files=length(file_list);

for diri=1:num_files

    % load individual Other standard average ERPs

    OtherStdfilename = file_list(diri).name; fprintf('Processing %s\n', OtherStdfilename);
    subject=extractBefore(OtherStdfilename,'_Other standard_avgERPs.mat');
    rootname = extractBefore(OtherStdfilename,'Other standard_avgERPs.mat');
    AllOtherStdtrialsfilename = [rootname 'Other standard_alltrialsERPs.mat'];

    % Gather all individual averages for Omission trials
    load(OtherStdfilename);
    GrandAvgContraFrontalERPOtherStd.(subject)=avgContraFrontalERP; GrandAvgContraS1ERPOtherStd.(subject)=avgContraS1ERP;
    GrandAvgIpsiFrontalERPOtherStd.(subject)=avgIpsiFrontalERP; GrandAvgIpsiS1ERPOtherStd.(subject)=avgIpsiS1ERP;
...
%% calculate grand averages
GrandAvgContraFrontalERPOtherStd.grandAvg=nanmean(GrandAvgContraFrontalERPOtherStd{:,:},2);
GrandAvgContraS1ERPOtherStd.grandAvg=nanmean(GrandAvgContraS1ERPOtherStd{:,:},2);
GrandAvgIpsiFrontalERPOtherStd.grandAvg=nanmean(GrandAvgIpsiFrontalERPOtherStd{:,:},2);
GrandAvgIpsiS1ERPOtherStd.grandAvg=nanmean(GrandAvgIpsiS1ERPOtherStd{:,:},2);

%% Save Omission traces to .mat file for script 7 Omission analysis
save('GrandAvgContraFrontalERPOtherStd.mat','GrandAvgContraFrontalERPOtherStd');
save('GrandAvgContraS1ERPOtherStd.mat','GrandAvgContraS1ERPOtherStd');
save('GrandAvgIpsiFrontalERPOtherStd.mat','GrandAvgIpsiFrontalERPOtherStd');
save('GrandAvgIpsiS1ERPOtherStd.mat','GrandAvgIpsiS1ERPOtherStd');

%% Save the tables to an Excel file
writetable(GrandAvgContraFrontalERPOtherStd, 'OtherStdERP.xlsx', 'Sheet', 'ContraFrontal','WriteRowNames', true);
writetable(GrandAvgContraS1ERPOtherStd, 'OtherStdERP.xlsx', 'Sheet', 'ContraS1','WriteRowNames', true);
writetable(GrandAvgIpsiFrontalERPOtherStd, 'OtherStdERP.xlsx', 'Sheet', 'IpsiFrontal','WriteRowNames', true);
writetable(GrandAvgIpsiS1ERPOtherStd, 'OtherStdERP.xlsx', 'Sheet', 'IpsiS1','WriteRowNames', true);

%% plot grand avgs
subplot(5,4,17)
plot(time,GrandAvgContraS1ERPOtherStd.grandAvg, 'k', 'LineWidth', 2); hold on;
xlabel('Time (ms)'); ylabel('Amplitude (ƵV)'); yline(0); ylim([-graphylimit graphylimit]);  xlim([-300 800]); xline(-200,':'); xline(0,':'); xline(200,':'); axis ij;
title('OtherStd Somatosensory contralateral cortex');
subplot(5,4,18)
plot(time,GrandAvgContraFrontalERPOtherStd.grandAvg, 'k', 'LineWidth', 2); hold on;
xlabel('Time (ms)'); ylabel('Amplitude (ƵV)'); yline(0); ylim([-graphylimit graphylimit]);  xlim([-300 800]); xline(-200,':'); xline(0,':'); xline(200,':'); axis ij;
title('OtherStd Frontal contralateral cortex');
subplot(5,4,19)
plot(time,GrandAvgIpsiFrontalERPOtherStd.grandAvg, 'k', 'LineWidth', 2); hold on;
xlabel('Time (ms)'); ylabel('Amplitude (ƵV)'); yline(0); ylim([-graphylimit graphylimit]);  xlim([-300 800]); xline(-200,':'); xline(0,':'); xline(200,':'); axis ij;
title('OtherStd Frontal ipsilateral cortex');
subplot(5,4,20)
plot(time,GrandAvgIpsiS1ERPOm.grandAvg, 'k', 'LineWidth', 2); hold on;
xlabel('Time (ms)'); ylabel('Amplitude (ƵV)'); yline(0); ylim([-graphylimit graphylimit]);  xlim([-300 800]); xline(-200,':'); xline(0,':'); xline(200,':'); axis ij;
title('OtherStd Somatosensory ipsilateral cortex');