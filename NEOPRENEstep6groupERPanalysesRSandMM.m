...
AllRSvalues=table('RowNames',{'ContraFrontal_Fam_peak','ContraFrontal_Control_peak','ContraS1_Fam_peak','ContraS1_Control_peak',...
    ...
    'ContraFrontal_Fam_latency','ContraS1_Fam_latency','IpsiFrontal_Fam_latency','IpsiS1_Fam_latency',...
    'ContraFrontal_Control_latency','ContraS1_Control_latency','IpsiFrontal_Control_latency','IpsiS1_Control_latency',...
...
for diri=1:num_files
    filename = file_list(diri).name;
    datafilename=file_list(diri).name; load(datafilename); % load individual RS results
    fprintf('Processing %s\n', datafilename);
    if contains(datafilename, '2Y_')
        subject=extractBefore(datafilename,'_2Y_RS.mat');
    elseif ~contains(datafilename, '2Y_')
    ...
    AllRSvalues.(subject)=[minContraFrontalERPFam;minContraFrontalERPControl;minContraS1ERPFam;minContraS1ERPControl;...
        ...
        ContraFrontalFamLatency;ContraS1FamLatency;IpsiFrontalFamLatency;IpsiS1FamLatency;...
        ContraFrontalControlLatency;ContraS1ControlLatency;IpsiFrontalControlLatency;IpsiS1ControlLatency;...
...
for subi=1:size(AllRSvalues,2)
    for factori=1:size(Factors,1)
        if contains(AllRSvalues.Properties.VariableNames{subi},Factors.Subject(factori))
            ...
            AllRSvalues{'log(Pain)',subi}=Factors.logPain(factori);
            AllRSvalues{'StimSide',subi}=Factors.StimSide(factori);
            AllRSvalues{'Sex',subi}=Factors.Sex(factori);
            AllRSvalues{'SDSC',subi}=Factors.SDSC(factori);
            ...
            AllRSvalues{'PC1_Actim',subi}=Factors.PC1_Actim(factori);
        end
    end
end
...
for diri=1:num_files
    ...
    load(datafilename); % load individual DevMM results
    fprintf('Processing %s\n', datafilename);
...
for subi=1:size(AllDevMMvalues,2)
    for factori=1:size(Factors,1)
        if contains(AllDevMMvalues.Properties.VariableNames{subi},Factors.Subject(factori))
            ...
            AllDevMMvalues{'log(Pain)',subi}=Factors.logPain(factori);
            AllDevMMvalues{'StimSide',subi}=Factors.StimSide(factori);
            AllDevMMvalues{'Sex',subi}=Factors.Sex(factori);
            AllDevMMvalues{'SDSC',subi}=Factors.SDSC(factori);
            ...
            AllDevMMvalues{'PC1_Actim',subi}=Factors.PC1_Actim(factori);
        end
    end
end
clear subi factori diri
...
for subi=1:size(AllPomMMvalues,2)
    for factori=1:size(Factors,1)
        if contains(AllPomMMvalues.Properties.VariableNames{subi},Factors.Subject(factori))
            ...
            AllPomMMvalues{'log(Pain)',subi}=Factors.logPain(factori);
            AllPomMMvalues{'StimSide',subi}=Factors.StimSide(factori);
            AllPomMMvalues{'Sex',subi}=Factors.Sex(factori);
            AllPomMMvalues{'SDSC',subi}=Factors.SDSC(factori);
            ...
            AllPomMMvalues{'PC1_Actim',subi}=Factors.PC1_Actim(factori);
        end
    end
end
clear subi factori diri
